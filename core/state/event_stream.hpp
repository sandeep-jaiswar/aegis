...
class event_stream {
  public:
    ...
    explicit event_stream(const event_stream_config& config = {}) noexcept
        : cfg(config), next_sequence(config.initial_sequence) {
        // Note: In production, would use custom allocator
        // For now, using compile-time fixed capacity
        // events_storage is raw memory and doesn't need initialization
    ...
    [[nodiscard]] event_stream_result push(const EventData& data, uint64_t timestamp_ns,
        ...
        if (events_count >= cfg.max_events) {
        ...
        new (&events_ptr()[events_count]) state_event<EventData>(meta, data);
        events_count++;

        // Update statistics
        statistics.total_events++;
        statistics.current_events = events_count;
        statistics.max_sequence = next_sequence;
        if (events_count == 1) {
            statistics.min_sequence = next_sequence;
        }
        statistics.bytes_allocated = events_count * sizeof(state_event<EventData>);
    ...
    [[nodiscard]] event_stream_result push(const state_event<EventData>& event) noexcept {
        ...
        if (!cfg.auto_sequence && events_count > 0) {
            const uint64_t last_seq = events_ptr()[events_count - 1].metadata.sequence;
        ...
        // Store event using placement new
        new (&events_ptr()[events_count]) state_event<EventData>(event);
        events_count++;

        // Update statistics
        statistics.total_events++;
        statistics.current_events = events_count;
        statistics.max_sequence = event.metadata.sequence;
        if (events_count == 1) {
            statistics.min_sequence = event.metadata.sequence;
        }
        statistics.bytes_allocated = events_count * sizeof(state_event<EventData>);

        // Update next_sequence to maintain ordering
        if (event.metadata.sequence >= next_sequence) {
            next_sequence = event.metadata.sequence + 1;
        }
    ...
    replay(const state_snapshot<StateData>& initial_state,
        ...
        state_snapshot<StateData> final_state =
            transition.apply_batch(initial_state, events_ptr(), events_count, trans_result);
    ...
    void reset() noexcept {
        ...
        events_count = 0;
        next_sequence = cfg.initial_sequence;
        statistics = event_stream_stats{};
    }
    ...
    [[nodiscard]] bool full() const noexcept {
        return events_count >= cfg.max_events;
    }
  ...
  private:
    event_stream_config cfg;
    // Use aligned storage to avoid default construction requirement
    alignas(state_event<EventData>) unsigned char events_storage[1024 *
                                                                 sizeof(state_event<EventData>)]{};
    size_t events_count{0};
    uint64_t next_sequence{0};
    mutable event_stream_stats statistics{}; // Mutable for replay count

    // Helper to get event array pointer
    [[nodiscard]] state_event<EventData>* events_ptr() noexcept {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
        return reinterpret_cast<state_event<EventData>*>(events_storage);
    }

    [[nodiscard]] const state_event<EventData>* events_ptr() const noexcept {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
        return reinterpret_cast<const state_event<EventData>*>(events_storage);
    }
};
