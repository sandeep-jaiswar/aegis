...
static int execute_build(const cli_config& config) {
    ...
    snprintf(configure_cmd, sizeof(configure_cmd), "cmake -B build -DCMAKE_BUILD_TYPE=%s%s",
             config.build_type,
    ...
    if (config.target != nullptr) {
        snprintf(build_cmd, sizeof(build_cmd), "cmake --build build --target %s%s", config.target,
                 config.verbose ? "" : " 2>&1 | grep -E '(Building|Linking|\\[.*%\\])'");
    } else {
        snprintf(build_cmd, sizeof(build_cmd), "cmake --build build%s",
                 config.verbose ? "" : " 2>&1 | grep -E '(Building|Linking|\\[.*%\\])'");
    }
    ...
    printf("\n[3/3] Build artifacts:\n");
    int ret = system("find build -type f \\( -name '*.a' -o -name '*.so' -o -name 'aegis_shell' -o "
                     "-name 'compile_commands.json' \\) -exec ls -lh {} \\;");
    (void)ret; // Intentionally ignore - this is informational output only

    if (config.show_artifacts) {
        printf("\nDetailed artifact listing:\n");
        ret =
            system("find build -type f ! -path '*/CMakeFiles/*' ! -name '*.o' -exec ls -lh {} \\;");
        (void)ret; // Intentionally ignore - this is informational output only
    }
...
static int execute_run(const cli_config& config) {
    ...
    if (config.record_path != nullptr) {
        ...
        snprintf(run_cmd, sizeof(run_cmd), "%s %s --record %s%s", shell_path, config.module_path,
                 config.record_path, config.verbose ? "" : " 2>&1");
    } else {
        snprintf(run_cmd, sizeof(run_cmd), "%s %s%s", shell_path, config.module_path,
                 config.verbose ? "" : " 2>&1");
    }
    ...
    if (config.record_path != nullptr && result == 0) {
        ...
        snprintf(stat_cmd, sizeof(stat_cmd),
                 "ls -lh %s 2>/dev/null || echo '(Recording file not created)'",
                 config.record_path);
...
static int execute_replay(const cli_config& config) {
    ...
    snprintf(replay_cmd, sizeof(replay_cmd), "%s %s --replay %s%s", shell_path, config.module_path,
             config.replay_path, config.verbose ? "" : " 2>&1");
...
static int execute_bench(const cli_config& config) {
    ...
    int ret = system(
        "find build/core/benchmark -type f -executable 2>/dev/null | while read -r bench; do echo "
    ...
    if (config.benchmark_name != nullptr) {
        ...
        const int path_len = snprintf(bench_path, sizeof(bench_path), "build/core/benchmark/%s",
                                      config.benchmark_name);
        ...
        char bench_cmd[4096];
        const int cmd_len = snprintf(bench_cmd, sizeof(bench_cmd), "%s%s", bench_path,
                                     config.verbose ? "" : " 2>&1");
